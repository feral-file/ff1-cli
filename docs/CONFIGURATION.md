...
- `playlist.role` (string): DP-1 signing role that travels with the private key. Defaults to `agent` if omitted. You can also set this via `PLAYLIST_ROLE` in `.env`. Guided `ff1 setup` only accepts the usual DP-1 signing roles (`agent`, `feed`, `curator`, `institution`, `licensor`). If you edit config by hand or override `--role`, that string is forwarded to **`dp1-js`** (`SignMultiEd25519`) without an extra enum check in ff1-cli.
