...
- `verify <file-or-url>` – Validate structure and verify signatures; legacy `signature` playlists accept `--public-key` (hex with optional `0x`, base64 SPKI DER, PEM, or 32-byte raw public key encoded as hex or base64)
